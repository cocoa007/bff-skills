...
var DLMM_CONTRACT_ADDR = "SP3B5B8HNE2N01GS0ZTMHPKV5DFN09JC7GBQZNTE";
var DLMM_CONTRACT_NAME = "dlmm-core-v-1-1";
var BFF_APP_BASE = "https://bff.bitflowapis.finance/api/app/v1";
var SENDER = "SP000000000000000000002Q6VF78";
var MIN_TVL_USD = 1000;
var BIN_SCAN_RADIUS = 15;
var FEE_EARNING_RADIUS = 1;
var CAPTURE_THRESHOLDS = {
  EXCELLENT: 80,
  GOOD: 60,
  FAIR: 40
};
var FALLBACK_STX_PRICE_USD = 0.8;
...
function parseUintResult(result) {
  if (!result?.result)
    return 0;
...
var BOLD = "\x1B[1m";
var DIM = "\x1B[2m";
function getRating(captureRate) {
  if (captureRate >= CAPTURE_THRESHOLDS.EXCELLENT)
    return "EXCELLENT";
  if (captureRate >= CAPTURE_THRESHOLDS.GOOD)
    return "GOOD";
  if (captureRate >= CAPTURE_THRESHOLDS.FAIR)
    return "FAIR";
  return "POOR";
}
var _poolCache = null;
async function fetchAllPools() {
  ...
  const pools = (data?.pools || data || []).map((p) => ({
    id: p.id ?? p.poolId ?? "?",
...
async function fetchStxPriceUsd() {
  try {
    ...
    for (const p of pools) {
      if (p.token0Symbol.toUpperCase() === "STX" && p.token0PriceUsd > 0)
        return p.token0PriceUsd;
      if (p.token1Symbol.toUpperCase() === "STX" && p.token1PriceUsd > 0)
        return p.token1PriceUsd;
    }
...
function computeFeeCaptureMetrics(bins, activeBinId, pool) {
  ...
  const activeBinConcentration = totalLiquidityUsd > 0 ? feeZoneLiquidityUsd / totalLiquidityUsd * 100 : 0;
  const liquidityUtilization = totalLiquidityUsd > 0 ? feeZoneLiquidityUsd / totalLiquidityUsd * 100 : 0;
  const feeRate = (pool.feeBps || 30) / 1e4;
  ...
  const adjustedFeeZone = scannedToTvlRatio > 0 ? feeZoneLiquidityUsd / scannedToTvlRatio : feeZoneLiquidityUsd;
  const feeCaptureRate = pool.tvlUsd > 0 ? Math.min(100, adjustedFeeZone / pool.tvlUsd * 100) : 0;
  ...
  const avgWeightedDistance = totalLiquidityUsd > 0 ? weightedDistance / totalLiquidityUsd : BIN_SCAN_RADIUS;
  let captureRateTrend;
  if (avgWeightedDistance <= 1.5) {
    captureRateTrend = "improving";
  } else if (avgWeightedDistance <= 4) {
...
function buildBinDistribution(bins, activeBinId) {
  const totalUsd = bins.reduce((s, b) => s + b.totalUsd, 0);
  return bins.filter((b) => b.totalUsd > 0).sort((a, b) => a.binId - b.binId).map((b) => ({
    binId: b.binId,
    usd: Math.round(b.totalUsd * 100) / 100,
    pctOfTotal: totalUsd > 0 ? Math.round(b.totalUsd / totalUsd * 1e4) / 100 : 0,
    inFeeZone: Math.abs(b.binId - activeBinId) <= FEE_EARNING_RADIUS,
    distFromActive: b.binId - activeBinId
  }));
}
function generateRecommendations(metrics, pool) {
  const recs = [];
  if (metrics.feeCaptureRate < CAPTURE_THRESHOLDS.FAIR) {
    recs.push(`Only ${formatPct(metrics.feeCaptureRate)} of fees captured. Consider rebalancing liquidity to concentrate near active bin ${pool.activeBinId || "N/A"}.`);
  }
  if (metrics.outerLiquidityUsd > metrics.feeZoneLiquidityUsd * 3) {
    recs.push(`${formatUsd(metrics.outerLiquidityUsd)} sits outside the fee zone \u2014 ${formatPct(100 - metrics.liquidityUtilization)} of liquidity is idle. Tighten your range for better capital efficiency.`);
  }
  if (metrics.feeLeakageDaily > 10) {
    recs.push(`Estimated ${formatUsd(metrics.feeLeakageDaily)}/day (${formatUsd(metrics.feeLeakageAnnualized)}/year) in fee leakage. Moving idle liquidity to the fee zone could recapture this.`);
  }
  if (metrics.captureRateTrend === "degrading") {
    recs.push("Fee capture is degrading \u2014 price has moved away from where liquidity is concentrated. Rebalance to follow the active bin.");
  }
  if (metrics.binsInFeeZone === 0) {
    recs.push("No liquidity detected in the fee-earning zone. All deployed capital is currently idle and earning zero fees.");
  }
  if (metrics.feeCaptureRate >= CAPTURE_THRESHOLDS.EXCELLENT) {
    recs.push("Excellent fee capture \u2014 liquidity is well-positioned. Maintain current range and monitor for active bin shifts.");
  }
  if (metrics.liquidityUtilization > 90 && metrics.feeCaptureRate > 70) {
    recs.push("High utilization with strong capture. This is an efficient LP position \u2014 focus on monitoring rather than rebalancing.");
  }
  if (recs.length === 0) {
...
function printAnalysisTable(analysis) {
  ...
  if (analysis.binDistribution.length > 0) {
    ...
    for (const entry of analysis.binDistribution) {
      ...
      console.log(`  ${String(entry.binId).padEnd(10)}${distLabel.padEnd(8)}${formatUsd(entry.usd).padEnd(14)}${formatPct(entry.pctOfTotal).padEnd(10)}${statusColor}${status}${RESET} ${DIM}${bar}${RESET}`);
    }
    console.log("");
  }
...
function printScanTable(analyses) {
  ...
  console.log(`${BOLD}\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550${RESET}`);
  console.log("");
  console.log(`  ${DIM}${"#".padEnd(6)}${"Pair".padEnd(18)}${"Capture".padEnd(12)}${"Rating".padEnd(12)}${"Util%".padEnd(10)}${"Leakage/d".padEnd(14)}${"Trend".padEnd(12)}${RESET}`);
  ...
  for (const a of sorted) {
    ...
    console.log(`  ${String(a.poolId).padEnd(6)}${a.pair.padEnd(18)}${color}${formatPct(m.feeCaptureRate).padEnd(12)}${m.captureRating.padEnd(12)}${RESET}${formatPct(m.liquidityUtilization).padEnd(10)}${formatUsd(m.feeLeakageDaily).padEnd(14)}${trendColor}${m.captureRateTrend}${RESET}`);
  }
  console.log("");
...
async function runDoctor() {
  ...
  try {
    const testMetrics = computeFeeCaptureMetrics([{ binId: 100, reserveX: 1000, reserveY: 1000, totalUsd: 500 }], 100, { tvlUsd: 1e4, volume24hUsd: 5000, feeBps: 30 });
...
async function analyzePool(poolId, pools) {
...
async function runAnalyze(poolIdStr) {
...
async function runScan() {
  ...
  console.log(JSON.stringify({
    ...
    rankings: analyses.sort((a, b) => b.metrics.feeCaptureRate - a.metrics.feeCaptureRate).map((a) => ({
      poolId: a.poolId,
      pair: a.pair,
...
async function runCompare(pool1Str, pool2Str) {
  ...
  if (captureDiff < 5) {
    ...
    reason = `Pool #${winner.poolId} (${winner.pair}) captures ${formatPct(captureDiff)} more fees. ` + `${loser.pair} leaks an additional ${formatUsd(leakageDiff)}/day vs ${winner.pair}.`;
  }
...
program2.name("hodlmm-fee-capture-rate").description("HODLMM Fee Capture Rate Analyzer \u2014 Measures what percentage of available trading fees a pool's LPs actually capture based on liquidity positioning relative to the active bin").version("1.0.0");
program2.command("doctor").description("Check API connectivity and dependencies").action(runDoctor);
program2.command("analyze <pool>").description("Analyze fee capture rate for a specific pool").action(runAnalyze);
program2.command("scan").description("Scan all pools and rank by fee capture rate").action(runScan);
program2.command("compare <pool1> <pool2>").description("Compare fee capture rates between two pools").action(runCompare);
program2.parse();
